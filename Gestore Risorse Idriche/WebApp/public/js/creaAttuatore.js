(async function (){
  ...
  let options = {
    ...
    console.log(getId)   
    let response = await fetch(`/aziende/${azienda.idAzienda}/campi/${getId}/attuatori`);
    const attuatori = await response.json();
    if(attuatori!= null)
    {
      for (let attuatore of attuatori)
      {
        tipo=attuatore.tipo;
      ...
      if(Object.keys(options).length == 0)
      {
        ...
        h3.innerHTML = "Sono gia' stati aggiunti tutti gli attuatori per questo campo!" ;
        c.appendChild(h3);
